...
describe('Gets all Least Favourite Programming languages', () => {
  it('selects all the options of programming languages from the database', async () => {
    const result = await request(server).get('/api/v1/least-fav-languages')
...
describe('Get total vote counts of the least favourite language different options', () => {
  it('Retireves all the instances of the different vote counts and counts them', async () => {
    const result = await request(server).get(
      '/api/v1/least-fav-languages/least-fav-votes',
    )
    expect(result.status).toBe(StatusCodes.OK)
    expect(result.body).toEqual([
      { language: 'C#', votes: 1 },
...
describe('Adds a vote to the least favourite programming languages', () => {
  it('adds an intance of the programming language selected to the database', async () => {
    const [postId] = '3'
    const result = await request(server).post(
      `/api/v1/least-fav-languages/${postId}`,
    )
